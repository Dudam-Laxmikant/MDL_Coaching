...
function AdminHome() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [noticeRes, totalRes] = await Promise.all([
          axios.get("http://localhost:8080/notice/notice"),
          axios.get("http://localhost:8080/TotalAdminDashboards"),
        ]);

        setnotice(noticeRes.data.data);
        settotalclass(totalRes.data.totalclass);
        settotalteacher(totalRes.data.totalteachers);
        settotalstudentes(totalRes.data.totalstudentes);
      } catch (err) {
        console.log("Error loading dashboard:", err);
      } finally {
        setTimeout(() => setLoading(false), 1000); // small delay for animation effect
      }
    };

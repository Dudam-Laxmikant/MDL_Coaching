...
function ShowAllTeachersList() {
  ...
  const filterTeachers = () => {
    let filtered = teachers.filter((teacher) => {
      ...
      const searchWords = searchName.toLowerCase().split(" ").filter(word => word);
      return (
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios.delete(`http://localhost:8080/teacher/deleteteacher/${id}`);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex flex-1 bg-[#454649] p-5 sm:p-7 md:p-14 min-h-screen">
        <div className="w-full max-w-6xl mx-auto bg-[#454649] p-6">
          ...
          <div className="mb-4 text-right">
            <Link to="/showclassTeacher/addteacher">
              <button className="px-4 py-2 bg-blue-600 text-white rounded shadow hover:bg-blue-700 transition">+ Add Teacher</button>
            </Link>
          </div>
                ...
                {filteredTeachers.map((teacher, index) => (
                  <tr key={index} className="border-b border-gray-300 hover:bg-[#454655] transition">
                    ...
                    <td className="p-4 flex flex-wrap justify-center gap-2">
                      <Link to="/showclassTeacher/showallteacherlist/updatetecherdetails">
                        <button className="p-2 bg-blue-500 text-white rounded shadow hover:bg-blue-600 transition flex items-center">
                          <FiEdit size={18} className="md:mr-1" /> <span className="hidden md:inline">Update</span>
                        </button>
                      </Link>
                      <Link to={`/showclassTeacher/showallteacherlist/showprofile/${teacher._id}`}>
                        <button className="p-2 bg-green-500 text-white rounded shadow hover:bg-green-600 transition flex items-center">
                          <FiUser size={18} className="md:mr-1" /> <span className="hidden md:inline">Profile</span>
                        </button>
                      </Link>
                      <Link to="/showclassTeacher/showallteacherlist/salarydetails">                           <button className="p-2 bg-yellow-500 text-white rounded shadow hover:bg-yellow-600 transition flex items-center">                             <FiDollarSign size={18} className="md:mr-1" />                             <span className="hidden md:inline">Salary</span>                           </button>                         </Link>
                      <button onClick={() => handleDelete(teacher._id)} className="p-2 bg-red-500 text-white rounded shadow hover:bg-red-600 transition flex items-center">
                        <FiTrash2 size={18} className="md:mr-1" /> <span className="hidden md:inline">Delete</span>
                      </button>
                    </td>
                  </tr>
                ))}
